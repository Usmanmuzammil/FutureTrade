...
class usercontroller extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = depositrequest::select('id','user_id','bank_id','accountholder','accountnumber','depositamount','status')->where('user_id',$id)->get();
    ...
    {
        if ($request->ajax()) {
            $data = TransactionTable::select('id','user_id','type','avaiable_amount','status','deposit_id','withdraw_id','investment_id')->where('user_id', $id)->get();
    ...
    public function Trade(Request $request, string $id)
    {
        if ($request->ajax()) {
            $data = Investment::select('id','user_id','scheme_ref_id','amount','end_date_timestamp','created_at')->where('user_id', $id)->get();
    ...
    public function withdrawRequest(Request $request, string $id)
    {
        if ($request->ajax()) {
            $data = Withdrawrequest::select('id','user_id','bank_id','accountholder','accountnumber','withdrawamount','status')->where('user_id', $id)->get();
